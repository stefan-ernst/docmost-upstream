...
/**
 * Generic editor resource controller. The route shape is provider/resource
 * specific, but outbound provider access remains fully declarative: handlers
 * call the resource manifest's search/resolve functions and never forward raw
 * arbitrary paths, methods, headers, or query strings from the browser.
 */
...
export class IntegrationResourceController {
  ...
  private requireResource(integrationId: string, resourceId: string) {
    const manifest = this.registry.get(integrationId);
    if (!manifest) {
